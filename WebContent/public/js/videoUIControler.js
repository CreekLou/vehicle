...
//动态增加广告浏览次数
function videoAdClicks(){
    var url = '/videoAdClicks.html';
    var xmlhttp=getajaxHttp();
    xmlhttp.onreadystatechange=function(){
        if(xmlhttp.readyState==4){
            //HTTP响应已经完全接收才调用
          ...
          success: function(data){
              ...
              if(viewModel === 1){
                  $.each(mode,function(i,video){
					  if(playingId === -1){
						  playingId = video.id; //获取播放视频id
						  playingTitle = video.name;
						  playingX = video.scaleX;
						  playingY = video.scaleY;
						  playingPriase = video.priase;
					  }
          ...
          success: function(data){
              ...
              if(viewModel === 1){
                  $.each(mode,function(i,video){
                      ...
                      $("#ul_info").append(
                          '<li id='+video.id+' title='+video.priase+'><a title="0" onclick="changeSrc('+videoPlayListId+','+video.id+',\''+video.name+'\','+video.scaleX+','+video.scaleY+')">'+
                              '<img src="'+iconPath+'">'+
                              '<p class="clickNumbers">播放：<strong><span id='+video.id+'>'+video.clicks+'<span></strong></p>'+
                              '<h2>'+video.name+'</h2>'+
                              '<p>'+video.description+'</p></a><div style="float:right;font-size:12px;position:absolute;right:50px;top:7px">'+video.duration+'</div>'+
                          '</li>'
                      );
                  ...
                  $.each(mode,function(i,video){
                      $("#ul_info").append(
                          '<li id='+video.id+' title='+video.priase+'><a title="0" onclick="changeSrc('+videoPlayListId+','+video.id+',\''+video.name+'\','+video.scaleX+','+video.scaleY+')">'+
                              '<h2>'+video.name+'</h2>'+
                              '<p class="clickNumbersSimple">播放：<strong><span id='+video.id+'>'+video.clicks+'<span></strong></p>'+
                          '</li>'
                      );
                      videoPlayListId++; //播放列表id自加1
                  });
...
function playlistInit(){
	myPlayer.playList(videos, {
	  ...
	  setPlayerSreen(playingX,playingY);
